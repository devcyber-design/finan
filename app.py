...
# --- Layout ---
# REMOVENDO 'layout="wide"' para melhor adaptação em telas menores
...
# Ajustado para layout padrão (não wide)
st.markdown("""
<style>
...
/* Diminui padding no topo e nas laterais do conteúdo principal */
section.main {
    padding-top: 0rem; 
    padding-right: 1rem;
    padding-left: 1rem;
...
for label, key in pages.items():
    ...
    if st.sidebar.button(label, use_container_width=True, type=button_type):
        ...
        # Isso garante que a página não fique presa com a sidebar aberta no celular.
        st.markdown("""
        <script>
...
# ----------------------------------------------------------------------------------
# --- BLOCO DE MÉTRICAS (Empilhado verticalmente no topo para mobile) ---
# ----------------------------------------------------------------------------------
...
if not df.empty:
    ...
    st.markdown("<h3 style='text-align: center; margin-top: 0; margin-bottom: 0.5rem;'>Resumo Total</h3>", unsafe_allow_html=True) 

    # Colunas para exibir as métricas horizontalmente em telas maiores
    col_balanco, col_receita, col_despesa = st.columns(3)

    # Métricas adaptadas para as colunas. No celular, elas serão empilhadas.
    with col_balanco:
        st.markdown(
    ...
    with col_receita:
        st.markdown(
            ...
            f"<h4 style='margin:0;font-size:12px;'>⬆️ Receitas</h4><h3 style='margin:0;font-size:16px;'>{format_currency(total_income)}</h3></div>",
            unsafe_allow_html=True,
        )
    with col_despesa:
        st.markdown(
            f"<div style='background:#DC143C;padding:8px;border-radius:8px;text-align:center;color:white; margin-bottom: 5px;'>"
            f"<h4 style='margin:0;font-size:12px;'>⬇️ Despesas</h4><h3 style='margin:0;font-size:16px;'>{format_currency(total_expense)}</h3></div>",
            unsafe_allow_html=True,
        )
    st.divider()
...
if st.session_state.page == "lancamento":
    ...
    with st.form("novo_lancamento"):
        ...
        # Colunas são mantidas para melhor alinhamento em desktops, mas empilham em mobile
...
# --- Página: VISÃO GERAL (Dashboard Principal) ---
    ...
    if df.empty:
        ...
        # Colunas para Gráficos: em telas pequenas, empilham-se naturalmente
        ...
        with col_graph1:
            ...
            # Altura ajustada para caber melhor no mobile
            fig.update_layout(height=300, margin=dict(t=50, b=10, l=10, r=10), uniformtext_minsize=8, uniformtext_mode='hide') 
...
# --- Página: ANÁLISE ---
    ...
    if df.empty:
        ...
        # Submenu para tipos de análise - Mantido o layout de 4 colunas para desktop
        # No mobile, estas colunas se empilham, o que é aceitável.
...
# --- Página: HISTÓRICO ---
    ...
    if df.empty:
        ...
        df_display = df.copy()
        df_display.reset_index(inplace=True, drop=False)
